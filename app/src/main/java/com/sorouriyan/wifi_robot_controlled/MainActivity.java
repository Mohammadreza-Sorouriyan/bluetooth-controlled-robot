package com.sorouriyan.wifi_robot_controlled;
...
public class MainActivity extends AppCompatActivity implements View.OnTouchListener {
    ...
    public boolean onTouch(View v, MotionEvent event) {
        ...
        if (v.getId() == R.id.up && event.getAction() == MotionEvent.ACTION_UP){
            sendData("s");
        }
        if (v.getId() == R.id.down && event.getAction() == MotionEvent.ACTION_UP){
            sendData("s");
        }
        if (v.getId() == R.id.right && event.getAction() == MotionEvent.ACTION_UP){
            sendData("s");
        }
        if (v.getId() == R.id.left  && event.getAction() == MotionEvent.ACTION_UP){
            sendData("s");
        }
...
//                Toast.makeText(getApplicationContext(), "Error : + " + e.getMessage(), Toast.LENGTH_LONG).show();
    ...
    private void sendData(String arrow) {
//        @SuppressLint("DefaultLocale") String data = "s" ;
        if (bluetoothSocket!=null) {
            try {
                Log.i("car", "one");
                bluetoothSocket.getOutputStream().write(arrow.getBytes());
                Log.i("car", "two");
            }
            catch (IOException e) {
                Toast.makeText(getApplicationContext(), "Couldn't send data to the device", Toast.LENGTH_LONG).show();
    ...
    private void getJoystickMove() {
        joystickRight.setOnMoveListener(new JoystickView.OnMoveListener() {
            ...
            public void onMove(int angle, int strength) {
                ...
                else if(angle > 292 && strength > 0) { // down right
